...
import { initialState } from '#app/routes/resume/reducer';
import steps from '#app/routes/resume/steps';
...
export default function(options, cbk) {
    ...
    const client = new ApiClient(result);
    let resumestate = initialState;

    if (result.user != null) {
      resumestate = initialState.merge({
        currentStep: result.user.signup_step,
        resumeCompleted: steps.length == result.user.signup_step,
        data: result.user.resume,
      });
    }
